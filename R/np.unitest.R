## Test for density equality for univariate data 13/02/10.
...
npunitest <- function(data.x = NULL,
  ...
  if(is.numeric(data.x) && (max(data.x) < min(data.y) || max(data.y) < min(data.x))) warning("non-overlapping empirical distributions")
  ...
  Srho.univar <- function(data.x,data.y,bw.x,bw.y) {
    if((ncol(data.frame(data.x)) != 1) || (ncol(data.frame(data.y)) != 1))  stop("data vectors must have one dimension only") 
    if(is.numeric(data.x)) {
  ...
  test.stat <- Srho.univar(data.x,data.y,bw.x,bw.y)
  ...
  if(bootstrap) {
    ...
    for(b in 1:boot.num) {
      ...
      resampled.stat[b] <- Srho.univar(data.null.x,data.null.y,bw.x,bw.y)
