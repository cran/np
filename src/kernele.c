...
/* Conduct the estimation */
    ...
    if(BANDWIDTH_den == 0)
    {
        ...
        for(j=0; j < num_obs_eval; j++)
        {
            ...
            pdf_stderr[j] = sqrt(pdf[j]*K_INT_KERNEL_P/prod_nh);
    ...
    else
    {
        ...
        for(j=0; j < num_obs_eval; j++)
        {
            ...
            pdf[j] = sum_ker;
            pdf_stderr[j] = sqrt(sum_ker_temp*K_INT_KERNEL_P);
...
/* Conduct the estimation */
    ...
    if(BANDWIDTH_den == 0)
    {
        ...
        for(j=my_rank*stride; (j < num_obs_eval) && (j < (my_rank+1)*stride); j++)
        {
            ...
            pdf_stderr[j-my_rank*stride] = sqrt(pdf[j-my_rank*stride]*K_INT_KERNEL_P/prod_nh);
    ...
    else
    {
        ...
        for(j=my_rank*stride; (j < num_obs_eval) && (j < (my_rank+1)*stride); j++)
        {
            ...
            pdf_stderr[j-my_rank*stride] = sqrt(sum_ker_temp*K_INT_KERNEL_P);
...
/* Generate bandwidth vector given scale factors, nearest neighbors, or lambda */
    ...
    if(kernel_bandwidth(
        ...
        num_reg_ordered,
        vector_scale_factor,
        matrix_X_continuous_bandwidth,            /* Not used */
        matrix_X_continuous_eval,                 /* Not used */
...
/* Gradient */
                ...
                *pointer_m = sum_y_ker/sum_ker;
                ...
                mean_stderr[j] = sqrt(temp_var * K_INT_KERNEL_P / sum_ker);
...
flawed. Now using local constant.*/
                ...
                mean_stderr[j] = sqrt(temp_var * K_INT_KERNEL_P / XTKX[0][0]);
...
/* Now compute the beast... */
                ...
                else
                {
                    ...
                    {
                        printf("\r                                                                                        ");
                        printf("\r** XTKX[%d] is singular: adding ridge factor in adding ridge factor",j);
                        printf("\n");
                        mat_dumpf( XTKX, "%g ");
                    }
...
/* Gradient */
                ...
                mean_stderr[j-my_rank*stride] = sqrt(temp_var * K_INT_KERNEL_P / sum_ker);
...
flawed. Now using local constant.*/
                ...
                mean_stderr[j-my_rank*stride] = sqrt(temp_var * K_INT_KERNEL_P / XTKX[0][0]);
...
/* Conduct the estimation */
    ...
    if(BANDWIDTH_den == 0)
    {
        ...
        for(j=0; j < num_obs_eval; j++)
        {
            ...
            pdf_stderr[j] = sqrt(pdf[j]*K_INT_KERNEL_P/prod_h);
...
/* stderr needs extra tmp I think */
            pdf_stderr[j] = sqrt(sum_ker_temp*K_INT_KERNEL_P/sum_ker_marginal);
...
/* Conduct the estimation */
    ...
    if(BANDWIDTH_den == 0)
    {
        ...
        for(j=my_rank*stride; (j < num_obs_eval) && (j < (my_rank+1)*stride); j++)
        {
            ...
            pdf_stderr[j-my_rank*stride] = sqrt(pdf[j-my_rank*stride]*K_INT_KERNEL_P/prod_h);
...
/* stderr needs extra tmp I think */
            pdf_stderr[j-my_rank*stride] = sqrt(sum_ker_temp*K_INT_KERNEL_P/sum_ker_marginal);
